...
def hfs_solver(Ra, dt, nx, plot_slu=False, porder=0):
    ...
    if Ra == 50:
        ...
        w = Function(W, 'hfs_ra%d_nx%d.xml' % (Ra-5, nx))
        w0= Function(W, 'hfs_ra%d_nx%d.xml' % (Ra-5, nx))
    ...
    bc2 = [DirichletBC(W.sub(2), u_bc, boundaries, 1, "geometric"), \
           DirichletBC(W.sub(2), u_bc, boundaries, 3, "geometric")]
    bc = bc1 + bc2
    ...
    File('hfs_ra%d_nx%d.xml' % (Ra, nx)) << w 
    return [nx, Ra, Nu]

nxs = [32, 48, 64]
Ras = range(50,155,5)
dt = 1.0e-3
output_handle = file('hfs_table.txt', 'w')
for nx in nxs:
    output ="-"*30 + "\n%8s | %8s | %8s\n" % ('nx', 'Ra', 'Nu')
    output_handle.write(output)
    for Ra in Ras:
        temp = hfs_solver(Ra, dt, nx, True)
